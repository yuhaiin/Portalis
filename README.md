...
The first start creates `/var/lib/portalis/web-auth.secret` with mode 0600. When no password is configured, TCP requests are passwordless only when both their peer address and HTTP `Host` are loopback. If you open the UI from a non-loopback address before setting a password, enter the contents of this setup-token file when prompted, then configure a 12+ character password in Settings. Once a password is configured, TCP requests must authenticate even when they arrive through loopback; this also protects a public reverse proxy forwarding to the local listener. Preserve the public `Host` when configuring a reverse proxy. The CLI uses `/run/portalis/control.sock` and is intended for root or a dedicated local `portalis` group.
